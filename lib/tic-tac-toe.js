...
//
// To persist state, call getJsonState(). To recreate a game from state, pass in this JSON in the constructor.
//
...
var T3Game = module.exports = function() {
    ...
    function move(cellIndex){
        ...
        var board = buildBoardFromState(_state);
        if (board[cellIndex] == ''){
    ...
    function newGame(skillLevel) {
        initGame(0, -1, skillLevel); // Human goes first.
    }
    ...
    function isInt(data) {
        return (typeof skillLevel==='number' && (skillLevel%1)===0);
    }
